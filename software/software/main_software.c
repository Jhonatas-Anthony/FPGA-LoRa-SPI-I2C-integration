...
void imprimedisplay(float temp, float umid) {
    ...
    char temp_buf[10];
    char umid_buf[10];
    sprintf(temp_buf, "%.1fC", temp);
    sprintf(umid_buf, "%.1f%%", umid);
    ...
    // ssd1306_WriteString("🌡", Font_6x8, White); // substitua por bitmap se não suportar UTF-8
    // ssd1306_SetCursor(25, 15);
    ssd1306_WriteString(temp_buf, Font_16x24, White);

    // Umidade (parte inferior)
    ssd1306_SetCursor(15, 38);
    // ssd1306_WriteString("Umid", Font_6x8, White);
    // ssd1306_SetCursor(60, 38);
    // ssd1306_WriteString("💧", Font_6x8, White); // idem
